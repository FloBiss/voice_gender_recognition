...
function(gender){
  ...
  dataset <- read.csv("newDataSet.csv")
  recordingData <- read.csv("my_voice.csv", row.names = 1)
  recordingData$label <- gender
  recordingData$X <- 'newInput'
  dataset <- rbind(dataset, recordingData)
  write.table(dataset, file = "newDataSet.csv", sep = ",")
...
#Extract data from .wav and write them into a CSV
writeCSVFromWAV <- function(){
  
  # Load data
  soundFile <- processFolder('recording')
  
  # Remove unused columns.
  soundFile$label <- ''
  soundFile$duration <- NULL
  soundFile$sound.files <- NULL
  soundFile$selec <- NULL
  soundFile$peakf <- NULL
  
  # Write out csv dataset.
  write.csv(soundFile, file = 'my_voice.csv')
}
